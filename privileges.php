...
<div class="">
	...
	$user_id = mysqli_real_escape_string($connect, $_POST['search']);
		...
		$check = mysqli_query($connect, "SELECT * FROM privileges WHERE role_id = '{$user_id}'");
		$count = mysqli_num_rows($check);
		if ($count > 0) {
			$msg = "Added Already";
		}else{
		$sert = "INSERT INTO privileges (role_id, edit, post, remove, admin, approve) VALUES('$user_id', '$edit', '$post', '$delet', '$admin', '$approve')";
	...
	$user_id = mysqli_real_escape_string($connect, $_POST['search']);
		...
		$up = mysqli_query($connect, "UPDATE privileges SET edit = '$edit', post = '$post', remove = '$delet', admin = '$admin', approve = '$approve' WHERE role_id = '{$user_id}'");
		if ($up) {
